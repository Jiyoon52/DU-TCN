...
class ResidualBlock(Layer):
    ...
    def __init__(self,
                 ...
                 norm_multiplier: float = 0.8,
                 gp_cov_momentum: float=-1.,
                 gp_cov_ridge_penalty: float=0.9,
                 gp_kernel_scale: float=0.9,
                 **kwargs):
            ...
            for k in range(2):  # dilated conv block.
                ...
                with K.name_scope(name):  # name scope used to make sure weights get unique names
                    ...
                    if self.use_weight_norm:
                        ...
                        with K.name_scope('norm_{}'.format(k)):
                            conv = tfm.nlp.layers.SpectralNormalization(conv, norm_multiplier=self.norm_multiplier) #0.8
...
class TCN(Layer):
    ...
    def __init__(self,
        ...
        if self.use_batch_norm + self.use_layer_norm + self.use_weight_norm + self.use_spectral_norm > 1:
    ...
    def get_config(self):
        ...
        config['num_inducing'] = self.num_inducing
        config['gp_cov_momentum'] = self.gp_cov_momentum
        config['gp_cov_ridge_penalty'] = self.gp_cov_ridge_penalty
        config['normalize_input'] = self.normalize_input
        config['scale_random_features'] = self.scale_random_features
        config['gp_kernel_scale'] = self.gp_kernel_scale
        ...
        config['use_spectral_norm'] = self.use_spectral_norm
        config['norm_multiplier'] = self.norm_multiplier
        config['kernel_initializer'] = self.kernel_initializer
        return config


def compiled_tcn_sngp(num_feat,  # type: int
                      num_classes,  # type: int
                      nb_filters,  # type: int
                      kernel_size,  # type: int
                      num_inducing, # type: int
                      gp_cov_momentum, #type: float
                      gp_cov_ridge_penalty, #type: float
                      normalize_input, #type: bool
                      scale_random_features, #type: bool
                      gp_kernel_scale, #type: float
    ...
    x = TCN(nb_filters, kernel_size, num_inducing, gp_cov_momentum, gp_cov_ridge_penalty, 
            normalize_input, scale_random_features, gp_kernel_scale,
            nb_stacks, dilations, padding,
            use_skip_connections, dropout_rate, return_sequences,
            activation, kernel_initializer, use_batch_norm, use_layer_norm,
            use_weight_norm, use_spectral_norm, norm_multiplier, name=name)(input_layer)
    ...
    if not regression:
        ...
        x = Dense(64, activation=tf.keras.layers.LeakyReLU(alpha=0.01))(x)
        
        GP = tfm.nlp.layers.RandomFeatureGaussianProcess(units=num_classes, 
                                                         num_inducing=num_inducing,#1024, 2048, 4096 
